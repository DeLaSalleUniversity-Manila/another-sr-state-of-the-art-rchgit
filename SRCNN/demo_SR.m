...
imfiles = glob('C:\MATLAB_work\SotA_DATA\Testing',{'*.bmp','*.png','*.jpg'});
psnr_bic = zeros(numel(imfiles));
psnr_srcnn = zeros(numel(imfiles));
...
psnr_srcnn(i) = compute_psnr(im_gnd,im_h);

%% show results
% imwrite(im_b, ['C:\MATLAB_work\SotA_DATA\Testing' 'Bicubic Interpolation' '.bmp']);
imwrite(im_h, ['' '.bmp']);
