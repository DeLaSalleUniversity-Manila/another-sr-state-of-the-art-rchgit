...
mse_sr = zeros(numel(imfiles),1);
% mse_b = zeros(numel(imfiles),1);
...
psnr_sr(i) = psnr(HR,croppedOriginal);
ssim_sr(i) = ssim(HR,croppedOriginal);
% bic = imresize(imresize(croppedOriginal, 0.5), 2);
% psnr_b(i) = psnr(bic,croppedOriginal);
% ssim_b(i) = ssim(bic,croppedOriginal);
%% Compute the mean-square error of reconstruction.
mse_sr(i) = sum((HR(:) - croppedOriginal(:)) .* (HR(:) - croppedOriginal(:))) / numel(HR);
% fprintf(1, 'Reconstruction Mean-square error: %f\n', mse);
end

for i = 1:numel(imfiles)
%% Visualize the results
disp(['SR For ' imfiles{i}]);
disp(['PSNR: ' psnr_sr(i)]);
disp(['SSIM: ' ssim_sr(i)]);
disp(['MSE: ' mse_sr(i)]);
fprintf('\n');

% disp(['Bicubic For ' imfiles{i}]);
% disp(['PSNR: ' psnr_sr(i)]);
% disp(['SSIM: ' ssim_sr(i)]);
% disp(['MSE: ' mse_sr(i)]);
% disp('');
